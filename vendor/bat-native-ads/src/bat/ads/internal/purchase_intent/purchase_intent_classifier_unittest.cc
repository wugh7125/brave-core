...
const std::vector<std::tuple<std::string, std::vector<std::string>, uint8_t>> kTestSearchQueries = {
  ...
  std::tuple<std::string, std::vector<std::string>, uint8_t>("https://www.bing.com/search?q=audi+a6+review+2020&form=QBLH&sp=-1&pq=audi+a6+review+2020&sc=6-19&qs=n&sk=&cvid=B145DDF1F2E74411977B6C54DF81333F", audi_a6_segments, 1),
...
TEST_F(AdsPurchaseIntentClassifierTest, ExtractsIntentSignal) {
  for (const auto& test_search_query: kTestSearchQueries) {
    ...
    // std::cout << "[TEST] intent signal - no. segments: " << intent_signal.segments.size();
    // std::cout << ", strength: " << intent_signal.strength << "\n";
