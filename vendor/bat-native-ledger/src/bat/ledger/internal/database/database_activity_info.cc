...
void GenerateActivityFilterBind(
  ...
  int column = 0;
  if (!filter->id.empty()) {
    BindString(command, column++, filter->id);
  }

  if (filter->reconcile_stamp > 0) {
    BindInt64(command, column++, filter->reconcile_stamp);
  }

  if (filter->min_duration > 0) {
    BindInt(command, column++, filter->min_duration);
  }

  if (filter->excluded != ledger::ExcludeFilter::FILTER_ALL &&
      filter->excluded !=
      ledger::ExcludeFilter::FILTER_ALL_EXCEPT_EXCLUDED) {
    BindInt(command, column++, static_cast<int32_t>(filter->excluded));
  }

  if (filter->excluded ==
      ledger::ExcludeFilter::FILTER_ALL_EXCEPT_EXCLUDED) {
    BindInt(command, column++,
        static_cast<int>(ledger::PublisherExclude::EXCLUDED));
  }

  if (filter->percent > 0) {
    BindInt(command, column++, filter->percent);
  }

  if (filter->min_visits > 0) {
    BindInt(command, column++, filter->min_visits);
  }
}
...
bool DatabaseActivityInfo::MigrateToV1(ledger::DBTransaction* transaction) {
  if (!DropTable(transaction, table_name_) || !CreateTableV1(transaction)) {
    return false;
  }
